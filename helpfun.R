...
read.vad <- function(path, lowess = FALSE, na.strings = "-9999.0"){
  
  filename <- (Sys.glob(path))
  file <- basename(filename)
  datetime <- ymd_hms(stringi::stri_sub(file, from = 10, to = 30))
  
  for (i in 1:length(filename)){
    temp <- read.csv(filename[i], sep = ";", dec = ".", na.strings = na.strings)
...
read.vad.radar <- function(path, lowess = TRUE){
...
razimuth <- function(lon, lat, lon0 = -60.537289, lat0 = -31.848438) {
  m <- matrix(c(lon, lat), ncol = 2)
